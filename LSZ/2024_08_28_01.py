#这是一个Python数据处理任务，F:\Experimental Results\Average_Bias_Rating\politician_average_rating\without_url-rating\1_without_url_retweet_user_counts\combined_with_screenname_and_bias.csv是一个映射文件，表头为retweet_origin_user_id,retweet_origin_username,count,screen_name,bias。我们需要用到的是第一列、第二列和第五列。第一列和第二列的类型为string，第五列只有Right，Left和Center三种值。Right对应1，Center对应0，Left对应-1。 我们现在有另外一系列文件，在F:\three_parts_output\without_url下面有15个月的文件夹，它们从2019年12月到2021年2月，文件名形如output_2019_12。每个文件夹里面都有若干个CSV，我们每次处理一个月内的文件。这些CSV的表头是：retweeted_time,retweeted_id,retweeted_user_id,retweeted_username,retweeted_user_location,retweeted_hashtags,retweet_time,retweet_id,retweet_expanded_urls_array,retweet_origin_user_id,retweet_origin_username,retweet_origin_user_location,retweet_origin_user_intro_expanded_url,retweet_origin_user_des_expanded_url,retweet_origin_retweet_count,retweet_origin_hashtags,quoted_time,quoted_id,quoted_expanded_urls_array,quoted_origin_user_id,quoted_origin_username,quoted_origin_user_location,quoted_origin_user_intro_expanded_url,quoted_origin_user_des_expanded_url,quoted_origin_retweet_count,quoted_origin_hashtags,retweeted_full_text,retweeted_origin_full_text,quoted_origin_full_text。我们只使用其中的retweeted_user_id和retweet_origin_user_id这两列。 下面是我们要做的数据处理，包含两部分。第一部分是，对于F:\three_parts_output\without_url下面每个月文件夹内的所有csv，我们想要统计如下数据，并生成一个表头为retweeted_user_id, retweet_times, total_bias_points, average_bias_points四列的CSV，最终得到15个CSV并写入本地。对于F:\three_parts_output\without_url下面一个月文件夹内的所有csv中的数据，如果retweet_origin_user_id在F:\Experimental Results\Average_Bias_Rating\politician_average_rating\without_url-rating\1_without_url_retweet_user_counts\combined_with_screenname_and_bias.csv中出现，那么就将对应的bias的映射值（-1 0或者1）加到retweeted_user_id对应的total_bias_points上，同时为retweet_times加1。最终对于每个月的所有retweeted_user_id，用total_bias_points除以retweet_times得到average_bias_points。 以下是第二部分的数据处理，请注意，不同于第一部分我们独立处理每个月的数据（并最终得到15个月的csv），第二部分中，我们跨15个月进行处理。我们希望最终得到一个csv，表头为retweet_origin_user_id和total_times。对于F:\three_parts_output\without_url下面十五个月文件夹内的所有csv，如果retweet_origin_user_id在F:\Experimental Results\Average_Bias_Rating\politician_average_rating\without_url-rating\1_without_url_retweet_user_counts\combined_with_screenname_and_bias.csv中排前十，那么我们就对相应的total_times加1。换言之，我们在统计F:\Experimental Results\Average_Bias_Rating\politician_average_rating\without_url-rating\1_without_url_retweet_user_counts\combined_with_screenname_and_bias.csv中排名前十名的retweet_origin_user_id每个究竟出现了多少次。
...
monthly_dir = r'F:\three_parts_output\single_url\external_url'
...
cross_month_df.to_csv(r'F:\three_parts_output\cross_month_results2.csv', index=False)
