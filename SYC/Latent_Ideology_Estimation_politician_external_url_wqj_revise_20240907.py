...
def create_adjacency_matrix(filepath):
    ...
    allUsers = np.array([])
    Urls = np.array([])
    df = pd.read_csv(filepath, dtype=str, usecols=['retweeted_user_id', 'retweet_origin_domain'])
    tmpusers = df['retweeted_user_id'].unique()
    tmpurls = df['retweet_origin_domain'].unique()
    # print(type(tmpusers))
    # print(type(tmpurls))
...
def calculate_positions(filepath):
    ...
    print(A.shape)
    A, deleted_rows, deleted_cols = matrixOptimization(A, 2)# 在此处修改需要筛选的转发次数
    print(A.shape)
...
def main():
    directory = r'F:\Intermediate Results\Simplyfied Forwarding relationship\Simplyfied Forwarding relationship_Politician\external_url'
    for i in range(2019, 2022):
        ...
        for j in range(1, 13):
            ...
            graph_user_position(user_positions, 'Bias', 'Score', 'number', rf"F:\Experimental Results\Matrix Decomposition Results\politician_results\external_url_politicians_and_users_bias_results_graph\{filename.split('.')[0]}_user.png")
            graph_influencer_position(influencer_positions, 'Bias', 'Score', 'number', rf"F:\Experimental Results\Matrix Decomposition Results\politician_results\external_url_politicians_and_users_bias_results_graph\{filename.split('.')[0]}_influencer.png")
            save_scores_to_csv(user_positions, influencer_positions, rf"F:\Intermediate Results\Matrix Decomposition Results\politician_results\external_url\{filename.split('.')[0]}_user.csv", rf"F:\Intermediate Results\Matrix Decomposition Results\politician_results\external_url\{filename.split('.')[0]}_influencer.csv")
