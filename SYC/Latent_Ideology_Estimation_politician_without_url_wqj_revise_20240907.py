...
#统计用户转发情况。
def forwardingCount(A, choice):
    row_sums = np.sum(A, axis=choice)
    row_sums = np.sort(row_sums)[::-1]
    print('test', row_sums)
    ...
    if choice == 1:
        ...
        print("矩阵的每一列的和：", row_sums)
        print("政客被转发次数统计：", sum_counts)
...
def create_adjacency_matrix(filepath):
    ...
    allUsers = np.array([])
    Urls = np.array([])
    df = pd.read_csv(filepath, dtype=str, usecols=['retweeted_user_id', 'retweet_origin_user_id'])
    tmpusers = df['retweeted_user_id'].unique()
    tmpurls = df['retweet_origin_user_id'].unique()
    # print(type(tmpusers))
    # print(type(tmpurls))
...
def main():
    ...
    for i in range(2019, 2022):
        ...
        for j in range(1, 13):
            ...
            sorted_user = np.sort(user_positions, axis=0)[::-1]
            sorted_inf = np.sort(influencer_positions)[::-1]
            print(sorted_user)
            print(sorted_inf)
