...
def get_bias_dict():
    ...
    # 转换第5列中的字符串
    first_column = first_column.map({'Left': -1, 'Right': 1, 'Center': 0}).dropna()# 和别的列不一样了
    # 创建映射字典
...
# 这个是对的！！！！！！！！！！！！！！！！！！！！！！！
def 在完整twitter_url里提取出每个月的政客能影响的用户_给用户打分的第一步(value_map):
    ...
    for month in months:
        # Example usage
        input_folder = rf'F:\three_parts_output\single_url\twitter_url\output_{month}'
        # input_folder = r'F:\code\WQJ\平均值被打上分的推文数量和矩阵分解之前的简化转发关系数量对不上_终于改对啦\test'#测试
        output_folder = rf'F:\Intermediate Results\Average_Bias_Rating\twitter_url_politician_influence_output'#测试
        # 保存详细转发数据到 Excel
        statistic_results_file = rf'F:\Intermediate Results\Average_Bias_Rating\twitter_url_politician_influence_output\平均值方法的简化转发关系统计_{month}.csv'
...
if __name__ == '__main__':
    ...
    在完整twitter_url里提取出每个月的政客能影响的用户_给用户打分的第一步(bias_dict)
