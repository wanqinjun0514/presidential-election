...
def process_quarterly_user_bias_scores():
    ...
    # 定义文件夹路径和每个季度包含的月份
    base_path = rf'F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating'
    quarter_months = [
    ...
    for i, months in enumerate(quarter_months):
        ...
        # 遍历季度中的每个月份
        for month in months:
            file_path = os.path.join(base_path, f'user_bias_scores_{month}.csv')
...
def find_common_users_between_quarters():
    ...
    for i in range(len(quarter_files) - 1):
        ...
        output_path = rf'F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating\sankey_plot\every_quarter\common_user_bias\common_users_{q1_str}_to_{q2_str}.csv'
...
def Construct_matrix_data_for_drawing_Sankey_diagrams_by_media():
    ...
    for date1, date2 in zip(date1s, date2s):
        # 载入数据
        file_path = rf"F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating\sankey_plot\every_quarter\common_user_bias\common_users_{date1}_to_{date2}.csv"

        output_transition_matrix_path = rf"F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating\sankey_plot\every_quarter\cross_table\transitions_sankey_{date1}_to_{date2}.csv"
...
def plot_quaterly_sankey_by_meida():
    ...
    date2s = ['2020_03_2020_04_2020_05', '2020_06_2020_07_2020_08', '2020_09_2020_10', '2020_11_2020_12',
              '2021_01_2021_02']
    for date1, date2 in zip(date1s, date2s):
        csv_file = rf'F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating\sankey_plot\every_quarter\cross_table\transitions_sankey_{date1}_to_{date2}.csv'
...
def quaterly_change_name():
    ...
    data_part_name = 'multiple_url'
    # 文件夹路径
    folder_path = rf'F:\Experimental Results\Average_Bias_Rating\media_average_rating\{data_part_name}-rating\sankey_plot\every_quarter\Sankey_plot'
