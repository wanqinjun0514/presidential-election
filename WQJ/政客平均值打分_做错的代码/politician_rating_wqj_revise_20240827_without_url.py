...
# 没考虑到quoted_origin_user_id字段
# # 第10列的索引为9
def process_folder_old(folder_path, value_map, output_file_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv'):
            ...
            df = pd.read_csv(file_path, dtype={2: str, 9: str, 19: str}, low_memory=False)

            # 提取第10列和第3列
            tenth_column = df.iloc[:, 9]  # 第10列的索引为9  转发的原帖用户id
...
def process_folder(folder_path, value_map, output_file_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv'):
            ...
            # 读取CSV文件，确保第3列、第10列和第20列为字符串类型
            df = pd.read_csv(file_path, dtype={2: str, 9: str, 19: str}, low_memory=False)

            # 提取第10列、第20列和第3列
            tenth_column = df.iloc[:, 9]   # 第10列的索引为9  转发的原帖用户id
            twentieth_column = df.iloc[:, 19]  # 第20列的索引为19      引用的原帖用户id
            third_column = df.iloc[:, 2]   # 第3列的索引为2           转帖用户id

            # 将第十列和第二十列的值根据映射字典转换
            tenth_mapped_values = tenth_column.map(value_map).fillna(0)  # 将NaN替换为0
            twentieth_mapped_values = twentieth_column.map(value_map).fillna(0)  # 将NaN替换为0
...
# output_base_folder_path = rf'F:\Experimental Results\Average_Bias_Rating\politician_average_rating\test_without'
