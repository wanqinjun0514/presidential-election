...
def get_bias_dict(file_path):
    # 读取CSV文件
    df = pd.read_csv(file_path, dtype={3: str, 4: str})
...
def process_folder(folder_path, value_map, output_file_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv'):
            ...
            # 这里要改成提取了twitter后面的用户名之后，这个用户名是否在构建的政客字典里，在的话就算政客标签被转发或者引用，成功影响到了用户

            # 提取第十列和第二十列中的用户名
            tenth_usernames = tenth_column.apply(lambda x: extract_username(str(x)))
            twentieth_usernames = twentieth_column.apply(lambda x: extract_username(str(x)))
...
base_folder_path = rf'F:\three_parts_output\single_url\twitter_url'
# base_folder_path = rf'F:\three_parts_output\test'

output_base_folder_path = rf'F:\Experimental Results\Average_Bias_Rating\politician_average_rating\twitter_url-rating'
