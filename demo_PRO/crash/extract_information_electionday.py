    ...
    with open(file_path, 'r', encoding='utf-8') as source_file, open(temp_file_path, 'w', encoding='utf-8') as temp_file:
        for line in source_file:
            ...
            try:
                ...
                with open("H:\\electionday\\line_error.txt", 'a', encoding='utf-8') as error_file:
...
def split_text_file(input_file_path, output_folder, count, path_record, lines_per_file=500000):
    ...
    with open(input_file_path, 'r', encoding='utf-8') as input_file:
        ...
        # 行数计数
        lines_written = 0
        output_file_path = os.path.join(output_folder, f'electionday-filter-ids-ok-{count}.txt')
            ...
            for line in input_file:
                ...
                if lines_written >= lines_per_file:
                    ...
                    with open(path_record, 'a', encoding='utf-8') as record_txt:
                        print_string = f"electionday-filter-ids-ok-{count}的行数：{lines_written}\n"
...
def batch_json_to_xlsx(num_txt, path_record):
    ...
    for i in range(7, num_txt + 1):  # 生成0_output.xlsx-num_output.xslx
...
#   批量拆分txt
def batch_split_text(num_txt, path_record):

    input_folder = f'H:\\electionday'  # 替换为你的输入文件的文件夹路径
    output_folder = f'H:\\electionday'  # 替换为你想要保存拆分文件的文件夹路径
    file_count = 0

    for i in range(1, num_txt+1):
        input_file_path = os.path.join(input_folder, f'electionday-filter-ids_0{i}-ok.txt')
        file_count = split_text_file(input_file_path, output_folder, file_count, path_record)
    return file_count
#   批量处理检测json错误
def batch_check_json(num_txt, path_record):

    for i in range(1, num_txt+1):
        print("-------------------------------------------------------------------------------------------------------")
        input_txt_file = f'H:\\electionday\\electionday-filter-ids_0{i}-ok.txt'
        ...
        with open(path_record, 'a', encoding='utf-8') as record_txt:
            print_string = f"electionday-filter-ids_0{i}-ok 正确行数：{true_count}\t错误行数：{error_count}\t空行数：{kong_count}\n"
            record_txt.write(print_string)
...
if __name__ == "__main__":
    ...
    #检查json是否有错误并处理
    # batch_check_json(file_in_num, record_data_path)
    # print("完成错误检查和处理!")
    # # 批量拆分大型txt文件
    # txt_num = batch_split_text(file_in_num, record_data_path)
    # print("完成拆分！")
    # # #批量生成n_output.xlsx
