    ...
    with open(file_path, 'r', encoding='utf-8') as source_file, open(temp_file_path, 'w', encoding='utf-8') as temp_file:
        for line in source_file:
            ...
            try:
                ...
                with open("H:\\republican\\line_error.txt", 'a', encoding='utf-8') as error_file:
...
def split_text_file(input_file_path, output_folder, count, file_num, path_record, lines_per_file=500000):
    ...
    with open(input_file_path, 'r', encoding='utf-8') as input_file:
        ...
        # 行数计数
        lines_written = 0
        output_file_path = os.path.join(output_folder, f'republican-candidate-filter-ids-0{file_num}-ok-{count}.txt')
            ...
            for line in input_file:
                ...
                if lines_written >= lines_per_file:
                    ...
                    with open(path_record, 'a', encoding='utf-8') as record_txt:
                        print_string = f"republican-candidate-filter-ids-0{file_num}-ok-{count}的行数：{lines_written}\n"
...
def batch_split_text(num_file, num_txt, path_record):
    ...
    input_folder = f'H:\\republican\\republican-candidate-filter-ids-0{num_file}'  # 替换为你的输入文件的文件夹路径
    output_folder = f'H:\\republican\\republican-candidate-filter-ids-0{num_file}'  # 替换为你想要保存拆分文件的文件夹路径
    file_count = 0

    for i in range(1, num_txt+1):
        input_file_path = os.path.join(input_folder, f'republican-candidate-filter-ids-0{num_file}_0{i}-ok.txt')
...
def batch_check_json(num_file, num_txt, path_record):
    ...
    for i in range(1, num_txt+1):
        print("-------------------------------------------------------------------------------------------------------")
        input_txt_file = f'H:\\republican\\republican-candidate-filter-ids-0{num_file}\\republican-candidate-filter-ids-0{num_file}_0{i}-ok.txt'
        ...
        with open(path_record, 'a', encoding='utf-8') as record_txt:
            print_string = f"republican-candidate-filter-ids-0{num_file}_0{i}-ok 正确行数：{true_count}\t错误行数：{error_count}\t空行数：{kong_count}\n"
            record_txt.write(print_string)
...
if __name__ == "__main__":
    ...
    # 记录过程中的结果
    record_data_path = f'H:\\republican\\republican-candidate-filter-ids-0{file_num}-txt-data.txt'
    ...
    # #批量生成n_output.xlsx
    print(txt_num)
    batch_json_to_xlsx(file_num, txt_num - 1, record_data_path)
    print("完成数据的提取!")
