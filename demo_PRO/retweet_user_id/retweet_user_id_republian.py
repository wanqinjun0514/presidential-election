...
def batch_extract_relationship(num_file, num_xlsx):
    record_txt = f'H:\\retweet_user_id\\republican\\{num_file}\\{num_file}.txt'

    for j in range(0, num_xlsx + 1):  # 生成0_output.xlsx-num_output.xslx
        input_xlsx = f"H:\\republican\\republican-candidate-filter-ids-0{num_file}-output\\republican-candidate-filter-ids-0{num_file}-ok-{j}-output.xlsx"
        output_xlsx = f'H:\\retweet_user_id\\republican\\{num_file}\\{num_file}-{j}.xlsx'
        print("-----------------------------------------------------------------------------------------------------")
        print(input_xlsx)
        print(output_xlsx)
        print(record_txt)
        ...
        with open(record_txt, 'a', encoding='utf-8') as rt:
            print_string = f"republican-{num_file}-{j}：line_num: {line_num}\n"
...
if __name__ == "__main__":
    ...
    for i in range(3, 18):  # 文件名字
        ...
        print("=====================================================================================================")
        output_folder = f'H:\\retweet_user_id\\republican\\{i}'
        # 检查文件夹是否存在
