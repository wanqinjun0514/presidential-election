...
def batch_extract_id_url(num_file, num_xlsx):
    record_txt = f'H:\\retweeted_id - url\\republican\\{num_file}\\{num_file}.txt'

    for i in range(0, num_xlsx + 1):  # 生成0_output.xlsx-num_output.xslx
        input_xlsx = f"H:\\republican\\republican-candidate-filter-ids-0{num_file}-output\\republican-candidate-filter-ids-0{num_file}-ok-{i}-output.xlsx"
        output_xlsx = f'H:\\retweeted_id - url\\republican\\{num_file}\\{num_file}-{i}.xlsx'
        print("-----------------------------------------------------------------------------------------------------")
        print(input_xlsx)
        print(output_xlsx)
        print(record_txt)
        ...
        with open(record_txt, 'a', encoding='utf-8') as rt:
            print_string = f"democratic-{num_file}-{i}：all_none_num: {all_none_num}\t\tretweet_num: {retweet_num}\t\tretweet_url: {retweet_url}\t\tquote_num: {quote_num}\t\tquote_url: {quote_url}\n"
...
if __name__ == "__main__":
    ...
    for i in range(3, 7):
        ...
        print("=====================================================================================================")
        output_folder = f'H:\\retweeted_id - url\\republican\\{i}'
        # 检查文件夹是否存在
        ...
        batch_extract_id_url(i, file_num[i])
