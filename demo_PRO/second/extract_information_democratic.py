        ...
        for line in f:
            ...
            data_list.append(
                {'retweeted_time': retweeted_time,                                                  # 转帖时间
                 ...
                 'quoted_time': quoted_time,                                                        # 被引用的推文创建时间
                 'quoted_id': quoted_id,                                                            # 被引用的推文id
                 'quoted_expanded_urls_array': quoted_expanded_urls_array,                          # 被引用的推文中的url
                 'quoted_origin_user_id': quoted_origin_user_id,                                    # 被引用的推文发布者id
                 'quoted_origin_user_location': quoted_origin_user_location,                        # 被引用的推文发布者位置
                 'quoted_origin_user_intro_expanded_url': quoted_origin_user_intro_expanded_url,    # 被引用的推文发布者设置中的url
                 'quoted_origin_user_des_expanded_url': quoted_origin_user_des_expanded_url,        # 被引用的推文发布者描述中的url
...
if __name__ == "__main__":
    ...
    for i in range(11, 18):
        ...
        # 记录过程中的结果
        record_data_path = f'H:\\democratic\\democratic-candidate-filter-ids-{i}-txt-data.txt'
