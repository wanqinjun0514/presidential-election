...
def extract_txt_to_xlsx(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            data_list.append(
                {'retweeted_time': retweeted_time,                                                  # 转帖时间                      A
                 'retweeted_id': retweeted_id,                                                      # 转贴推文id                    B
                 'retweeted_user_id': retweeted_user_id,                                            # 转贴用户id                    C
                 'retweeted_user_location': retweeted_user_location,                                # 转贴用户位置                   D

                 'retweet_time': retweet_time,                                                      # 被转贴的推文创建时间             E
                 'retweet_id': retweet_id,                                                          # 被转贴的推文id                  F
                 'retweet_expanded_urls_array': retweet_expanded_urls_array,                        # 被转贴的推文中的url               G
                 'retweet_origin_user_id': retweet_origin_user_id,                                  # 被转贴的推文发布者id           H
                 'retweet_origin_user_location': retweet_origin_user_location,                      # 被转贴的推文发布者位置           I
                 'retweet_origin_user_intro_expanded_url': retweet_origin_user_intro_expanded_url,  # 被转贴的推文发布者设置中的url      J
                 'retweet_origin_user_des_expanded_url': retweet_origin_user_des_expanded_url,      # 被转贴的推文发布者描述中的url      K
                 'retweet_origin_retweet_count': retweet_origin_retweet_count,                      # 被转贴的推文的转贴次数              L

                 'quoted_time': quoted_time,                                                        # 被引用的推文创建时间             M
                 'quoted_id': quoted_id,                                                            # 被引用的推文id                  N
                 'quoted_expanded_urls_array': quoted_expanded_urls_array,                          # 被引用的推文中的url               O
                 'quoted_origin_user_id': quoted_origin_user_id,                                    # 被引用的推文发布者id           P
                 'quoted_origin_user_location': quoted_origin_user_location,                        # 被引用的推文发布者位置           Q
                 'quoted_origin_user_intro_expanded_url': quoted_origin_user_intro_expanded_url,    # 被引用的推文发布者设置中的url      R
                 'quoted_origin_user_des_expanded_url': quoted_origin_user_des_expanded_url,        # 被引用的推文发布者描述中的url      S
                 'quoted_origin_retweet_count': quoted_origin_retweet_count                         # 被引用的推文的转贴次数             T
                })
            true_line += 1
...
def batch_json_to_xlsx(num_txt, path_record):
    output_folder = f'H:\\electionday\\electionday-filter-ids-output'
    # 检查文件夹是否存在
    ...
    for i in range(0, num_txt + 1):  # 生成0_output.xlsx-num_output.xslx
        input_txt_file = f"H:\\electionday\\electionday-filter-ids-ok-{i}.txt"
        output_xlsx_file = f"H:\\electionday\\electionday-filter-ids-ok-{i}.xlsx"
        print("-----------------------------------------------------------------------------------------------------")
        ...
        with open(path_record, 'a', encoding='utf-8') as record_txt:
            print_string = f"electionday-filter-ids-ok-{i}总行数：{all_count}\t有效行数：{true_count}\t无效行数：{all_count - true_count}\n"
...
if __name__ == "__main__":
    ...
    # 记录过程中的结果
    record_data_path = f'H:\\electionday\\electionday-filter-ids-txt-data.txt'
    if not os.path.isfile(record_data_path):
        # 如果文件不存在，创建文件
        with open(record_data_path, 'w') as file:
            print(f"文件 '{record_data_path}' 不存在，已创建。")
    ...
    batch_json_to_xlsx(13, record_data_path)
