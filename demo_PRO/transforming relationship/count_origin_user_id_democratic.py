...
def batch_extract_relationship(num_file, num_xlsx):
    record_txt = f'H:\\tranforming_relationship\\democratic_count\\{num_file}\\{num_file}.txt'

    for i in range(0, num_xlsx + 1):  # 生成0_output.xlsx-num_output.xslx
        input_xlsx = f"H:\\tranforming_relationship\\democratic\\{num_file}\\{num_file}-{i}.xlsx"
        output_xlsx = f'H:\\tranforming_relationship\\democratic_count\\{num_file}\\{num_file}-{i}.xlsx'
        print("-----------------------------------------------------------------------------------------------------")
        print(input_xlsx)
        print(output_xlsx)
        print(record_txt)
        ...
        with open(record_txt, 'a', encoding='utf-8') as rt:
            print_string = f"democratic-{num_file}-{i}：line_num_before: {line_num_before}\t\tline_num_after: {line_num_after}\n"
...
if __name__ == "__main__":
    ...
    for i in range(3, 18):  # 文件名字

        print("=====================================================================================================")
        output_folder = f'H:\\tranforming_relationship\\democratic_count\\{i}'
        # 检查文件夹是否存在
