...
def batch_url_count(num_xlsx):

    record_txt = f'H:\\url_statistics\\electionday\\electionday.txt'

    for i in range(0, num_xlsx + 1):  # 生成0_output.xlsx-num_output.xslx
        input_xlsx = f"H:\\electionday\\electionday-filter-ids-ok-{i}.xlsx"
        output_xlsx = f'H:\\url_statistics\\electionday\\{i}.xlsx'
        print("-----------------------------------------------------------------------------------------------------")
        print(input_xlsx)
        print(output_xlsx)
        print(record_txt)
        ...
        with open(record_txt, 'a', encoding='utf-8') as rt:
            print_string = f"electionday-{i}：all_none_num: {all_none_num}\t\tretweet_num: {retweet_num}\t\tquote_num: {quote_num}\t\tall_no_none_num: {all_no_none_num}\t\tall_no_none_index: {all_no_none_index}\n"
